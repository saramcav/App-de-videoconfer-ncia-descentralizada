...
class Server:
    ...
    def handle_client(self, conn, addr):
        ...
        while conectado:
            ...
            if msg[0] == DISCONNECT_MSG: #se a mensagem recebida for para desconectar o cliente
                ...
                msg = self.validate_login(name, password)
                print(self.clients_table())    
            ...
            elif msg[0] == UPDATE_MSG:
                name, new_port = msg[1].split(',')
                msg = self.update_client_register(name, new_port)
                print(self.clients_table())
    ...
    def update_client_register(self, name, new_port):
        msg = '{UPDATE_ACK}::=Sua porta foi atualizada com sucesso!'
        client_register = self._clients.get(name)
        client_register.set_reception_port(new_port)
        client_register.set_online_status(True)
        return msg
        
    def update_client(self, name, password):
        client_register = self._clients.get(name)
        is_valid_login = self.validate_user(client_register, password)
        if is_valid_login:
            msg = f'{UPDATE_MSG}::=Bem vindo/a novamente! Insira sua nova porta de recepção!'
        else:
            msg = f'{FORCED_DISCONNECTION_MSG}::=Senha inválida. Você será desconectado...'
