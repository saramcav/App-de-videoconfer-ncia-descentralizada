...
class Server:
    ...
    def add_client_register(self, name, ip, reception_port): #função que adiciona um novo registro de cliente no servidor e retorna uma mensagem
        client_name = self._clients.get(name)
        client_addr = None
        for client, client_register in self._clients.items():
                if client_register.get_ip() == ip and client_register.get_reception_port() == reception_port:
                    client_addr = client_register
        ...
        if client_name is None:
        ...
        elif client_name.get_status() == True:
            ...
            client_name.set_ip(ip)
            client_name.set_reception_port(reception_port) 
            client_name.set_status(True)
    ...
    def query_user(self, client_name): #função que busca um nome de usuário solicitado pelo cliente e retorna uma mensagem
        ...
        elif client != None and client.get_status() == False:
